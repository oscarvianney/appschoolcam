	...
	if user != course.user:
		...
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				p = Page.objects.create(title=title, content=content, video=video ,user=user)
...
@login_required
def EditPage(request, page_id,course_id, module_id):
	user = request.user
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			page.files = form.cleaned_data.add('files')
			page.video = form.cleaned_data.get('video')
			page.save()
